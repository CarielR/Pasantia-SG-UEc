	...
	public function generateRules($table) {
		...
		foreach ($table->columns as $column) {
			if ($column->isPrimaryKey && $table->sequenceName !== null)
				continue;
			if(!$column->allowNull)	
                                $columnRequired[]=$column->name;
			if (!(!$column->allowNull && $column->defaultValue === null))
				$null[] = $column->name;
		}
      		if($columnRequired!==array())
                    $required[]="array('".implode(', ',$columnRequired)."', 'required')";

                if ($null !== array())
			$rules[] = "array('" . implode(', ', $null) . "', 'default', 'setOnEmpty' => true, 'value' => null)";
		return array_merge($required,parent::generateRules($table), $rules );
	}
